# I'm Matías Bazán! 👋

<div align="center">

# 🚀 Desarrollador Full Stack: Flutter, Node.js & PostgreSQL

[![Portfolio](https://img.shields.io/badge/Portafolio-Mi%20Sitio%20Web-007ACC?style=for-the-badge&logo=vercel&logoColor=white)](https://portafolio-matias-bazan-web.vercel.app/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Matias%20Baz%C3%A1n-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/mat%C3%ADas-baz%C3%A1n-470a38227/)

---

</div>

## 👨‍💻 Sobre Mí

¡Hola! Soy **Matías Bazán**, Desarrollador Full Stack especializado en **Flutter**, **Node.js** y **PostgreSQL**. Me enfoco en crear soluciones de software sólidas, eficientes y bien estructuradas tanto para plataformas web como móviles.

Actualmente me encuentro investigando y aplicando **Spec Kit SDD (Spec Driven Development)** para optimizar el diseño de especificaciones, arquitecturas y desarrollo guiado por requerimientos.

- 📍 **Ubicación:** Argentina
- 🔬 **En exploración / aprendizaje:** Spec Kit SDD (Spec Driven Development)
- 🎯 **Objetivo:** Construir productos digitales escalables con código limpio y arquitecturas robustas.

---

## 🛠️ Tech Stack & Herramientas

### **Desarrollo Core & Backend**
![Flutter](https://img.shields.io/badge/Flutter-02569B?style=for-the-badge&logo=flutter&logoColor=white)
![Dart](https://img.shields.io/badge/Dart-0175C2?style=for-the-badge&logo=dart&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white)

### **Herramientas & Metodologías**
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)
![Spec Kit SDD](https://img.shields.io/badge/Spec_Driven_Development-Spec_Kit-4A90E2?style=for-the-badge)

---

## 📈 Estadísticas de GitHub

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=iamMatiasBazan&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" alt="Estadísticas de GitHub" width="48%" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=iamMatiasBazan&layout=compact&theme=tokyonight&hide_border=true" alt="Lenguajes más usados" width="48%" />
</div>
